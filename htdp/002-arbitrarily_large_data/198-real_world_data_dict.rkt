...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname 198-real_world_data_dict) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; Exercise 198
...
; A list List-of-Dictionaries is one of:
; - '()
; - (cons Dictionary LoD)


(check-expect (most-frequent.v2 DICTIONARY-AS-LIST) (most-frequent DICTIONARY-AS-LIST))
; Dictionary -> Letter-Counts
; Presents the letter that occurs most often as the first one
(define (most-frequent.v2 dict)
  (most-frequent-helper (words-by-first-letter dict) (make-letter-counts "" 0)))

(check-expect (most-frequent-helper
               (list (list "aardvark" "abelung")
                     (list "massachussets" "michigan")
                         (list "bee"))
               (make-letter-counts "" 0))
              (make-letter-counts "a" 2))
(define (most-frequent-helper dict lc)
  (cond
    [(empty? dict) lc]
    [(> (length (first dict)) (letter-counts-count lc))
     (most-frequent-helper (rest dict)
                           (make-letter-counts
                            (first (explode (first (first dict))))
                            (length (first dict))))]
    [else
          (most-frequent-helper (rest dict) lc)]))

(check-expect (words-by-first-letter (list "aardvark" "abelung" "massachussets" "michigan" "bee"))
              (list
               (list "michigan" "massachussets")
               (list "bee")
               (list "abelung" "aardvark")))
; Dictionary -> List-of-Dictionaries
; produces a list of Dictionarys, one per Letter.
(define (words-by-first-letter dict)
  (dict-by-letter dict LETTERS '()))

(check-expect (dict-by-letter (list "aardvark" "abelung" "massachussets" "michigan" "bee")
                            LETTERS
                            '())
              (list
               (list "michigan" "massachussets")
               (list "bee")
               (list "abelung" "aardvark")))
; Dictionary Letter List-of-Dictionaries -> List-of-dictionaries
; produces a Dictionary for a single letter
(define (dict-by-letter dict letter lod)
  (cond
    [(empty? letter) lod]
    [(empty? (first-letter dict (first letter) '()))
     (dict-by-letter dict (rest letter) lod)]
    [else
     (dict-by-letter dict (rest letter) (cons (first-letter dict (first letter) '()) lod))]))
  

(check-expect (first-letter  '() "a" '()) '())
(check-expect (first-letter (list "aardvark" "abelung" "massachussets" "michigan" "bee")
                            "a"
                            '())
              (list "abelung" "aardvark"))
(check-expect (first-letter (list "aardvark" "abelung" "massachussets" "michigan" "bee")
                            ...
                            '())
              (list "bee"))
; Dictionary Letter Dictionary -> Dictionary
; produces a Dictionary for a single letter
(define (first-letter dict letter new-dict)
  (cond
    [(empty? dict) new-dict]
    [(starts-with? letter (first dict))
     (first-letter (rest dict) letter (cons (first dict) new-dict))]
    [else
     (first-letter (rest dict) letter new-dict)]))
